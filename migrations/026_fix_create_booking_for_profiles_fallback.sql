-- ============================================
-- Migration 026: Fix create_booking to support both schemas
-- Makes create_booking work with profiles table when students/teachers don't exist
-- ============================================
...
BEGIN
  ...
  -- Send notifications (using the name variables we set earlier)
  INSERT INTO notifications (user_id, type, title, subtitle, data) VALUES
    (p_teacher_id, (CASE WHEN v_booking_status = 'awaiting_payment' THEN 'BOOKING_PENDING' ELSE 'BOOKING_CONFIRMED' END)::notification_type,
      ...
      jsonb_build_object('booking_id', v_booking_id, 'student_name', v_student_name, 'subject', p_subject,
  ...
  INSERT INTO notifications (user_id, type, title, subtitle, data) VALUES
    (p_student_id, (CASE WHEN v_booking_status = 'awaiting_payment' THEN 'BOOKING_PENDING' ELSE 'BOOKING_CONFIRMED' END)::notification_type,
      ...
      jsonb_build_object('booking_id', v_booking_id, 'teacher_name', v_teacher_name, 'subject', p_subject,
        'start_at', p_start_at, 'duration', p_duration_minutes, 'status', v_booking_status));
...
DO $$
BEGIN
  RAISE NOTICE '✅ Migration 026 completed - create_booking now supports both students/teachers and profiles tables';
END $$;
