-- ============================================
-- Migration 005: Enhance Booking Schema
...
-- Rename/adjust existing columns for consistency
DO $$ 
BEGIN
  -- Rename 'price' to match total_price if needed
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='bookings' AND column_name='price') THEN
    -- Update total_price from price if total_price is null
    UPDATE bookings SET total_price = price WHERE total_price IS NULL;
    -- We keep both for now to avoid breaking existing code
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS audit_log (
  ...
  entity TEXT NOT NULL, -- 'booking', 'payment', 'refund', etc.
  entity_id UUID,
  meta JSONB,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- RLS for audit_log (admin only - for future)
ALTER TABLE audit_log ENABLE ROW LEVEL SECURITY;

-- For now, only allow viewing by the actor
...
-- Credit transactions: for ledger queries
...
-- ============================================
-- Function: Create booking with credit deduction
-- ============================================
...
BEGIN
  ...
  IF v_hours_until_lesson >= 24 THEN
    v_refund_amount := v_booking.total_price - COALESCE(v_booking.credits_applied, 0); -- Full refund (excluding credits already applied)
  ELSIF v_hours_until_lesson >= 12 THEN
    v_refund_amount := (v_booking.total_price - COALESCE(v_booking.credits_applied, 0)) * 0.5; -- 50% refund
  ELSE
...
-- Add new notification types for booking flow
DO $$ BEGIN
...
COMMENT ON COLUMN bookings.mode IS 'Lesson delivery mode: online, at student location, or at teacher location';
COMMENT ON COLUMN bookings.duration_minutes IS 'Lesson duration: 45, 60, or 90 minutes';
COMMENT ON COLUMN bookings.credits_applied IS 'Amount of student credits applied to this booking';
COMMENT ON COLUMN bookings.coupon_code IS 'Promotional coupon code applied';
COMMENT ON COLUMN bookings.discount_amount IS 'Discount amount from coupon';
COMMENT ON COLUMN bookings.timezone IS 'Timezone of the booking (e.g., Asia/Jerusalem)';
COMMENT ON COLUMN bookings.source IS 'Source of booking: mobile, web, admin';
COMMENT ON COLUMN bookings.student_level IS 'Student academic level: elementary, middle_school, high_school, academic, other';

COMMENT ON TABLE refunds IS 'Refund records for cancelled bookings';
COMMENT ON TABLE audit_log IS 'Audit trail for important actions (bookings, payments, refunds)';

-- ============================================
-- Sample data for testing
-- ============================================

-- Note: Only run this section if you want test data
-- Commented out by default to avoid duplicates

/*
-- Create a test booking (requires existing teacher and student)
INSERT INTO bookings (
  teacher_id, student_id, subject_id,
  start_at, end_at,
  mode, duration_minutes,
  price_per_hour, total_price,
  status, notes, timezone
) VALUES (
...
BEGIN
  ...
  RAISE NOTICE 'Added: refund processing function';
END $$;
