-- ============================================
-- Migration 035: Fix Availability Functions to Use Teachers Table
-- Updates open_day, close_day, and upsert_availability_slots to use teachers table instead of profiles
-- ============================================
...
BEGIN
  ...
  -- Get teacher timezone from teachers table (fixed)
  SELECT timezone INTO v_teacher_tz FROM teachers WHERE id = p_teacher_id;
...
-- Grant execute permissions (already granted, but keeping for clarity)
...
BEGIN
  ...
  RAISE NOTICE 'Fixed functions: upsert_availability_slots, close_day, open_day to use teachers table instead of profiles';
END $$;
